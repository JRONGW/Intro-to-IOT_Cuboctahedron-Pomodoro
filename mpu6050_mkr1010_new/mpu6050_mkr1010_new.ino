...
#define I2CDEVLIB_WIRE_BUFFER_LENGTH 256  // SAMD Wire buffer
...
void mpu_setup()
{
  ...
  Serial.println(mpu.getDeviceID(), HEX); // expect 0x34 for MPU6050 (0b110100)
...
void mpu_loop()
{
  static unsigned long lastBeat = 0, lastWait = 0;

  // show a heartbeat every second
  ...
  if (!mpuInterrupt && fifoCount < packetSize) {
    if (millis() - lastWait > 500) {
      lastWait = millis();
      Serial.println(F("waiting for DMP INT (check INT -> D2 wiring)"));
    }
    return;
  }
  ...
  if ((mpuIntStatus & 0x10) || fifoCount == 1024) {
    ...
    // CSV for Serial Plotter: yaw,pitch,roll (deg) per line
