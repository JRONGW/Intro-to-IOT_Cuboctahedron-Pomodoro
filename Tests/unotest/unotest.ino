/***************************************************************
 * UNO + MPU-6050 (GY-521) — Cuboctahedron Face Detection
 ...
 * Power the GY-521 at 3.3V so its I2C pull-ups are 3.3V.
 ***************************************************************/
...
// Thresholds & timing
const float ENTER_TH = 0.80f;          // was 0.85
const float EXIT_TH  = 0.72f;          // was 0.80
...
void setup() {
  ...
  if (!mpu.testConnection()) {
    Serial.println(F("MPU6050 not connected. Check wiring (3.3V power, A4/A5, GND)."));
    delay(3000);
  }
...
void loop() {
  ...
  // Re-arm logic: once confirmed, require score to fall below EXIT_TH
  // for REARM_DROP_MS before allowing another confirmation (even same face).
  if (announcedThisFace) {
    if (currentFace.score < EXIT_TH) {
      ...
      belowActive = false; // still above, keep waiting
    }
  }

  if (!haveCandidate) {
    // No valid face yet—reset dwell candidate if we weren't armed
  ...
  if (dwellCandidate.idx == -1 || !sameFace(dwellCandidate, currentFace)) {
    ...
    dwellStartMs   = nowMs;
    announcedThisFace = false;     // allow new confirm after change
  } else {
    ...
    if (!announcedThisFace && elapsed >= DWELL_MS) {
      ...
      announcedThisFace = true;    // wait for re-arm
    }
  }
